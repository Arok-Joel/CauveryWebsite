  ...
  try {
    ...
    if (employee.employeeRole === "EXECUTIVE_DIRECTOR" && employee.leadsTeam) {
      ...
      if (role !== "EXECUTIVE_DIRECTOR") {
        return NextResponse.json({
          error: "Cannot change role: This Executive Director is currently leading a team. Please assign a new team leader first."
        }, { status: 400 })
      }
    }
    ...
    if (role === "EXECUTIVE_DIRECTOR") {
      ...
      if (teamMembership?.teamId) {
        return NextResponse.json({
          error: "Cannot promote to Executive Director: Employee is currently a member of a team. Please remove them from the team first."
        }, { status: 400 })
      }
    }
