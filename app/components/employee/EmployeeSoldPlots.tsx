...
import { BadgeIndianRupee, ChevronDown, LayoutGrid, UserCheck, Loader2, Download } from 'lucide-react';
...
interface EmployeeSoldPlotsProps {
  initialCommissions: Commission[];
  employeeId: string;
  employeeName: string;
}

export function EmployeeSoldPlots({ initialCommissions, employeeId, employeeName }: EmployeeSoldPlotsProps) {
  ...
  const downloadCSV = () => {
    ...
    const rows = commissions.map(commission => [
      commission.soldPlot.plotNumber,
      commission.amount.toString(),
      (parseFloat(commission.percentage.toString()) * 100).toFixed(1),
      format(new Date(commission.soldPlot.soldAt), 'yyyy-MM-dd'),
      commission.soldPlot.size,
      commission.soldPlot.price,
      commission.soldPlot.dimensions,
      commission.soldPlot.facing,
      commission.soldPlot.plotAddress,
      commission.soldPlot.customerName,
      commission.soldPlot.phoneNumber,
      commission.soldPlot.email,
      commission.soldPlot.aadhaarNumber,
      commission.soldPlot.address
    ].map(value => {
    ...
    // Create blob and download with employee name in filename
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    const sanitizedEmployeeName = employeeName.replace(/[^a-z0-9]/gi, '-').toLowerCase();
    link.setAttribute('href', url);
    link.setAttribute('download', `sold-plots-${sanitizedEmployeeName}-${plotsFilter}.csv`);
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-[1fr_1fr_1fr_1fr_40px] gap-6 items-center py-2 border-b">
              <div className="text-sm font-medium text-muted-foreground">Plot Number</div>
              <div className="text-sm font-medium text-muted-foreground text-center">Commission</div>
              <div className="text-sm font-medium text-muted-foreground text-center">Percentage</div>
              <div className="text-sm font-medium text-muted-foreground">Sale Date</div>
              <div></div>
            </div>
